...
namespace QuantumTek.QuantumDialogue.Editor
{
    public class QD_DialogueEditor : EditorWindow
    {
            ...
            else
            {
                ...
                if (e.type == EventType.MouseDown && e.button == 1 && !connectingNodes)
                {
                    ...
                    if (!clicked)
                    {
                        ...
                        menu.AddItem(new GUIContent("Add Variables"), false, ContextCallback, "Add Variables");
                        menu.ShowAsContext();
                        e.Use();
                    }
                    else
                ...
                for (int i = 0; i < nodeCount; ++i)
                {
                    ...
                    else if (node.Type == QD_NodeType.Variables)
                    {
                        QD_VariableNode realNode = db.GetVariableNode(node.ID);
                        node.Window = GUI.Window(node.ID, node.Window, realNode.DrawWindow, realNode.WindowTitle);
                    }
                    node.DrawKnobs();
                }
        ...
        public void ContextCallback(object obj)
        {
            ...
            else if (command == "Add Variables")
                db.CreateNode(QD_NodeType.Variables, mouse + scroll);
            ...
            else if (command == "Delete Node")
            {
                ...
                if (node.Type == QD_NodeType.Speaker)
                    ...
                    db.VariableNodes.RemoveAt(db.GetVariableNodeIndex(selectedNode.ID));
                db.Nodes.RemoveAt(i);
                if (node.Type == QD_NodeType.Speaker)
                {
                ...
                {
                    i = db.DataDB.GetVariableIndex(selectedNode.ID);
                    db.DataDB.Variables.RemoveAt(i);
                }
                if (db.Nodes.Count == 0)
                    db.NextID = 0;
            }
