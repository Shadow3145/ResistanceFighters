...
namespace QuantumTek.QuantumDialogue.Editor
{
    ...
    public class QD_NodeDB : ScriptableObject
    {
        ...
        public List<QD_VariableNode> VariableNodes = new List<QD_VariableNode>();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public QD_VariableNode GetVariableNode(int id)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public int GetVariableNodeIndex(int id)
        {
            int nodeCount = VariableNodes.Count;
            for (int i = 0; i < nodeCount; ++i)
                if (VariableNodes[i].ID == id)
                    return i;
            return -1;
        }
        ...
        public void CreateNode(QD_NodeType type, Vector2 position)
        {
            ...
            {
                var variableNode = new QD_VariableNode(id, type, position.x, position.y);
                node.Window.width = variableNode.Window.width;
                node.Window.height = variableNode.Window.height;
                node.Inputs = variableNode.CloneKnobs(QD_KnobType.Input);
                node.Outputs = variableNode.CloneKnobs(QD_KnobType.Output);
                node.AllowedInputs = variableNode.AllowedInputs;
                node.AllowedOutputs = variableNode.AllowedOutputs;

                var variableData = new QD_Variable(id);
                DataDB.Variables.Add(variableData);
                variableNode.Data = variableData; 
